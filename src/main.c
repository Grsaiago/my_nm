...
int main(int argc, char **argv) {
	...
	for (int arg_idx = 1; arg_idx < argc; arg_idx++) {
		...
		if (map_file(argv[arg_idx], &mapped_file) != 0) {
			ft_printf("%s failed to map file '%s': %s\n", argv[0],
					  argv[arg_idx], strerror(errno));
			continue;
		}
		if (new_elf_file(&mapped_file, &elf) != 0) {
			ft_printf("%s failed to load ELF file '%s', are you sure it's an "
					  "ELF file?\n",
					  argv[0], argv[arg_idx]);
		...
		case (0):
			break;
		case (1):
			ft_printf("%s: %s: no symbol table\n", argv[0], argv[arg_idx]);
			continue;
		case (2):
			ft_printf("%s: %s: no string table, that's a stripped binary\n",
					  argv[0], argv[arg_idx]);
			continue;
		}
		...
		if (symbol_list == NULL) {
			ft_printf("%s: %s: failed to parse symbol list\n", argv[0],
					  argv[arg_idx]);
			return (-1);
		}
		...
		for (SymbolList *curr_symbol = symbol_list; curr_symbol != NULL;
			...
			if (curr_symbol->value == 0) {
				...
				ft_printf("%x %c %s\n", curr_symbol->value, curr_symbol->digit,
						  curr_symbol->name);
			}
		}
